...
export namespace Option {
	 ...
	 */
	export const toUndefined = <A>(data: Option<A>): A | undefined => isSome(data) ? data.value : undefined;

	/**
	 * Creates a Option from a possibly undefined value.
	 * Returns None if undefined, Some otherwise.
	 */
	export const fromUndefined = <A>(value: A | undefined): Option<A> => value === undefined ? none() : some(value);
	 ...
	 * ```
	 */
	export const fromResult = <E, A>(data: Result<E, A>): Option<A> => Result.isOk(data) ? some(data.value) : none();
