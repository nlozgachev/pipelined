...
export namespace Optional {
   ...
   */
  export const fold =
    <S, A>(opt: Optional<S, A>) => <B>(onNone: () => B, onSome: (a: A) => B) => (s: S): B => {
      const val = opt.get(s);
  ...
  export const andThen =
    <A, B>(inner: Optional<A, B>) => <S>(outer: Optional<S, A>): Optional<S, B> =>
      make(
        (s) => {
   ...
   * ```
   */
  export const andThenLens =
    <A, B>(inner: Lens<A, B>) => <S>(outer: Optional<S, A>): Optional<S, B> =>
      make(
        (s) => {
          const mid = outer.get(s);
