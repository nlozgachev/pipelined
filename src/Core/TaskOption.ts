...
export namespace TaskOption {
	 ...
	 */
	export const map = <A, B>(f: (a: A) => B) => (data: TaskOption<A>): TaskOption<B> => Task.map(Option.map(f))(data);
	...
	export const chain = <A, B>(f: (a: A) => TaskOption<B>) => (data: TaskOption<A>): TaskOption<B> =>
		Task.chain((option: Option<A>) => Option.isSome(option) ? f(option.value) : Task.resolve(Option.none()))(data);
	 ...
	 */
	export const fold = <A, B>(onNone: () => B, onSome: (a: A) => B) => (data: TaskOption<A>): Task<B> =>
	 ...
	 */
	export const tap = <A>(f: (a: A) => void) => (data: TaskOption<A>): TaskOption<A> => Task.map(Option.tap(f))(data);
