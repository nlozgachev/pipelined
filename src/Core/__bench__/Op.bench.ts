...
describe("op-sync-transformations", () => {
	bench("1. (current) Op.map + Op.chain + Op.fold", () => {
		const outcome = Op.ok(42);
		void pipe(
			outcome,
			Op.map((x: number) => x * 2),
			Op.chain((x: number) => Op.ok(x + 1)),
			Op.fold((err: unknown) => `err:${String(err)}`, () => "nil", (val: number) => `ok:${val}`),
		);
	});

	bench("2. manual inline outcome checks", () => {
		const outcome = Op.ok(42) as Op.Outcome<string, number>;
		const s1 = outcome.kind === "OpOk" ? Op.ok(outcome.value * 2) : outcome;
		const s2 = s1.kind === "OpOk" ? Op.ok(s1.value + 1) : s1;
