...
test("Maybe.map — identity law", () => {
	fc.assert(
		fc.property(arbMaybe, (m) => {
			expect(Maybe.map((x: number) => x)(m)).toEqual(m);
		}),
	);
});
...
test("Maybe.map — composition law", () => {
	fc.assert(
		fc.property(arbMaybe, fc.integer(), fc.integer(), (m, a, b) => {
			const f = (x: number) => x + a;
			const g = (x: number) => x * b;
			expect(Maybe.map(f)(Maybe.map(g)(m))).toEqual(Maybe.map((x: number) => f(g(x)))(m));
		}),
	);
});
...
test("Maybe.chain — left identity", () => {
	fc.assert(
		fc.property(fc.integer(), (a) => {
			...
			expect(Maybe.chain(f)(Maybe.some(a))).toEqual(f(a));
		}),
	);
});
...
test("Maybe.chain — right identity", () => {
	fc.assert(
		fc.property(arbMaybe, (m) => {
			expect(Maybe.chain(Maybe.some)(m)).toEqual(m);
		}),
	);
});
...
test("Maybe.chain — associativity", () => {
	fc.assert(
		fc.property(arbMaybe, fc.integer(), (m, threshold) => {
			...
			expect(Maybe.chain(f)(Maybe.chain(g)(m))).toEqual(
				Maybe.chain((x: number) => Maybe.chain(f)(g(x)))(m),
			);
		}),
	);
});
...
test("Maybe.chain — short-circuits on None", () => {
	fc.assert(
		fc.property(fc.integer(), (a) => {
			expect(Maybe.chain((_: number) => Maybe.some(a))(Maybe.none())).toEqual(Maybe.none());
		}),
	);
});
...
test("Maybe.filter — always-true predicate is identity on Some", () => {
	fc.assert(
		fc.property(arbSome, (m) => {
			expect(Maybe.filter(() => true)(m)).toEqual(m);
		}),
	);
});
...
test("Maybe.filter — always-false predicate gives None on Some", () => {
	fc.assert(
		fc.property(arbSome, (_m) => {
			expect(Maybe.filter(() => false)(Maybe.some(0))).toEqual(Maybe.none());
		}),
	);
});
...
test("Maybe.filter — None passes through unchanged", () => {
	fc.assert(
		fc.property(arbNone, (m) => {
			expect(Maybe.filter(() => true)(m)).toEqual(Maybe.none());
		}),
	);
});
...
test("Maybe.fromPredicate — always-true gives Some with original value", () => {
	fc.assert(
		fc.property(fc.integer(), (n) => {
			expect(Maybe.fromPredicate((_: number) => true)(n)).toEqual(Maybe.some(n));
		}),
	);
});
...
test("Maybe.fromPredicate — always-false gives None", () => {
	fc.assert(
		fc.property(fc.integer(), (n) => {
			expect(Maybe.fromPredicate((_: number) => false)(n)).toEqual(Maybe.none());
		}),
	);
});
