...
test("Validation.map — identity law", () => {
	fc.assert(
		fc.property(arbValidation, (v) => {
			expect(Validation.map((x: number) => x)(v)).toEqual(v);
		}),
	);
});
...
test("Validation.map — composition law", () => {
	fc.assert(
		fc.property(arbValidation, fc.integer(), fc.integer(), (v, a, b) => {
			const f = (x: number) => x + a;
			const g = (x: number) => x * b;
			expect(Validation.map(f)(Validation.map(g)(v))).toEqual(
				Validation.map((x: number) => f(g(x)))(v),
			);
		}),
	);
});
...
test("Validation.ap — Valid(f) + Valid(a) = Valid(f(a))", () => {
	fc.assert(
		fc.property(fc.integer(), fc.integer(), (n, delta) => {
			const vf = Validation.valid<string, (x: number) => number>((x: number) => x + delta);
			const va = Validation.valid<string, number>(n);
			expect(Validation.ap(va)(vf)).toEqual(Validation.valid(n + delta));
		}),
	);
});
...
test("Validation.ap — Invalid(f) + Invalid(a) accumulates both error lists", () => {
	fc.assert(
		fc.property(fc.string(), fc.string(), (e1, e2) => {
			const vf: Validation<string, (x: number) => number> = Validation.invalid(e1);
...
test("Validation.getOrElse — returns fallback on Invalid", () => {
	fc.assert(
		fc.property(arbInvalid, fc.integer(), (v, fallback) => {
...
test("Validation.recover — identity on Valid", () => {
	fc.assert(
		fc.property(arbValid, (v) => {
			expect(Validation.recover((_) => Validation.valid(-999))(v)).toBe(v);
		}),
	);
});
...
test("Validation.fromPredicate — always-true gives Valid with original value", () => {
	fc.assert(
		fc.property(fc.integer(), (n) => {
			expect(
				Validation.fromPredicate((_: number) => true, () => "bad")(n),
			).toEqual(Validation.valid(n));
		}),
	);
});
...
test("Validation.product — two Valid produces Valid tuple", () => {
	fc.assert(
		fc.property(fc.integer(), fc.string(), (n, s) => {
			expect(
				Validation.product(Validation.valid<string, number>(n), Validation.valid<string, string>(s)),
			).toEqual(Validation.valid([n, s]));
		}),
	);
});
	...
	fc.assert(
		fc.property(fc.string(), fc.string(), (e1, e2) => {
			const result = Validation.product(
				Validation.invalid(e1) as Validation<string, number>,
