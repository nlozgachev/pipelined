...
const arbDict = fc
	.array(fc.tuple(fc.string({ minLength: 1 }), fc.integer()), { maxLength: 8 })
	.map((pairs) => Dict.fromEntries(pairs));
...
test("Dict.fromEntries → Dict.entries — every entry is found via lookup", () => {
	fc.assert(
		fc.property(
			fc.array(fc.tuple(fc.string({ minLength: 1 }), fc.integer()), { maxLength: 8 }),
			(pairs) => {
				const m = Dict.fromEntries(pairs);
				const entries = Dict.entries(m);
				entries.forEach(([k, v]) => {
					expect(Dict.lookup(k)(m)).toEqual(Maybe.some(v));
				});
			},
		),
	);
...
test("Dict.map — identity law", () => {
	fc.assert(
		fc.property(arbDict, (m) => {
			expect(Dict.map((x: number) => x)(m)).toEqual(m);
		}),
	);
});
...
test("Dict.map — composition law", () => {
	fc.assert(
		fc.property(arbDict, fc.integer(), fc.integer(), (m, a, b) => {
			const f = (x: number) => x + a;
			const g = (x: number) => x * b;
			expect(Dict.map(f)(Dict.map(g)(m))).toEqual(Dict.map((x: number) => f(g(x)))(m));
		}),
	);
});
...
test("Dict.filter(always true) — identity", () => {
	fc.assert(
		fc.property(arbDict, (m) => {
			expect(Dict.filter(() => true)(m)).toEqual(m);
		}),
	);
});
...
test("Dict.filter(always false) — empty result", () => {
	fc.assert(
		fc.property(arbDict, (m) => {
			expect(Dict.filter(() => false)(m)).toEqual(Dict.empty());
		}),
	);
});
...
test("Dict.insert — inserted key is immediately found via lookup", () => {
	fc.assert(
		fc.property(arbDict, fc.string({ minLength: 1 }), fc.integer(), (m, k, v) => {
			expect(Dict.lookup(k)(Dict.insert(k, v)(m))).toEqual(Maybe.some(v));
		}),
	);
});
...
test("Dict.remove — removed key is not found via lookup", () => {
	fc.assert(
		fc.property(arbDict, fc.string({ minLength: 1 }), (m, k) => {
			expect(Dict.lookup(k)(Dict.remove(k)(m))).toEqual(Maybe.none());
		}),
	);
});
...
test("Dict.union — result contains all keys from both maps", () => {
	fc.assert(
		fc.property(arbDict, arbDict, (m1, m2) => {
