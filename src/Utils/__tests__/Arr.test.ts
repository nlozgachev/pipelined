...
Deno.test("scan - returns running totals of a sum", () => {
	assertEquals(
		pipe([1, 2, 3], Arr.scan(0, (acc, n) => acc + n)),
		[1, 3, 6],
	);
});

Deno.test("scan - returns empty array for empty input", () => {
	assertEquals(pipe([], Arr.scan(0, (acc: number, n: number) => acc + n)), []);
});

Deno.test("scan - does not include the initial value in output", () => {
	assertEquals(pipe([5], Arr.scan(100, (acc, n) => acc + n)), [105]);
});

Deno.test("scan - output length equals input length", () => {
	const result = pipe([1, 2, 3, 4], Arr.scan(0, (acc, n) => acc + n));
	assertStrictEquals(result.length, 4);
});

Deno.test("scan - works with non-numeric accumulators", () => {
	assertEquals(
		pipe(["a", "b", "c"], Arr.scan("", (acc, s) => acc + s)),
		["a", "ab", "abc"],
	);
});
...
Deno.test("splitAt - splits array at a given index", () => {
	assertEquals(pipe([1, 2, 3, 4], Arr.splitAt(2)), [[1, 2], [3, 4]]);
});

Deno.test("splitAt - index 0 gives empty before and full after", () => {
	assertEquals(pipe([1, 2, 3], Arr.splitAt(0)), [[], [1, 2, 3]]);
});

Deno.test("splitAt - index equal to length gives full before and empty after", () => {
	assertEquals(pipe([1, 2, 3], Arr.splitAt(3)), [[1, 2, 3], []]);
});

Deno.test("splitAt - index beyond length clamps to end", () => {
	assertEquals(pipe([1, 2], Arr.splitAt(10)), [[1, 2], []]);
});

Deno.test("splitAt - negative index clamps to 0", () => {
	assertEquals(pipe([1, 2, 3], Arr.splitAt(-5)), [[], [1, 2, 3]]);
});

Deno.test("splitAt - empty array returns two empty arrays", () => {
	assertEquals(pipe([], Arr.splitAt(2)), [[], []]);
});
