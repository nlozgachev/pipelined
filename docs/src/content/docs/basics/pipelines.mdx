...
`Maybe.from.nullable` is passed directly as a function reference without an arrow wrapper.

`Maybe.map((u) => u.name)` is invoked with only a mapping function. It returns a function waiting for the `Maybe` value, which `pipe` supplies. This is enabled by data-last currying.
