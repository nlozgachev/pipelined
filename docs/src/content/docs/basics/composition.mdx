...
`compose` is the right-to-left counterpart of `flow`. While `flow(f, g)` executes `f` first and `g` second, `compose(g, f)` runs them in reverse order, matching traditional mathematical notation `(g ∘ f)`:
...
Piping raw values through standard functions works well for clean, linear logic. However, real-world applications frequently require optional branches, safe error handling, object reshaping, or asynchronous steps. Rather than forcing you to write noisy inline arrow functions, the `pipe` combinator provides several specialized extension helpers.
...
Symmetrically to `pipe`, the `flow` combinator provides extension helpers that compile these branching, catching, and asynchronous behaviors into reusable, deferred functions.
...
For debugging, performance auditing, and background tasks, `tap` provides several specialized, configuration-based side-effect helpers.

#### Printing values with tap.log

`tap.log` logs the piped value to the console. You can optionally supply a `label` to prefix the log output, a custom `logger` destination, or a custom string `formatter` (which defaults to `JSON.stringify` for objects):
...
pipe(
  42,
  tap.log(), // logs: 42
  tap.log({ label: "Total Score" }), // logs: [Total Score]: 42
  tap.log({
    label: "Custom Format",
    formatter: (n) => `Value is ${n * 2}`,
  }), // logs: [Custom Format]: Value is 84
);
```

#### Detailed object inspection with tap.inspect

For deeply nested objects, `tap.log` might output truncated representations. `tap.inspect` formats objects with infinite depth by default, utilizing Node's `util.inspect` engine under the hood:

```ts
pipe(
  { user: { name: "Alice", details: { age: 30, roles: ["admin"] } } },
  tap.inspect({ label: "User Payload", depth: 2 })
);
// logs: [User Payload]: { name: 'Alice', details: { age: 30, roles: [Array] } }
```

#### Background operations with tap.async

To trigger asynchronous operations (like telemetry hits, audits, or database writes) without blocking the pipeline or turning it into an async promise chain, use `tap.async`. It runs the promise in the background, intercepts any rejections, and routes them to `onError` (defaulting to `console.error`):
...
pipe(
  ...
  tap.async(
    async (id) => {
      await saveAnalyticsSession(id);
    },
    { onError: (err) => logError(err) }
  )
); // returns userId immediately; analytics save runs in background
```

#### Timing steps with tap.time

`tap.time` measures the execution duration of a side-effect function. It supports both synchronous and asynchronous operations (for promises, the duration is tracked until the promise resolves or rejects).

The configuration object requires either a `label` (to log to the console automatically) or an `onFinish` callback (to route the typed `Duration` elsewhere), which are mutually exclusive to avoid conflicts:

```ts
import { Duration } from "@nlozgachev/pipelined/types";

// Scenario A: Auto-log to console
pipe(
  data,
  tap.time(syncCalculation, { label: "heavy-math" })
); // logs: [heavy-math]: 12ms

// Scenario B: Custom APM callback receiving Duration
pipe(
  data,
  tap.time(asyncApiFetch, {
    onFinish: (duration) => {
      metrics.send("api.duration_ms", Duration.to.milliseconds(duration));
    }
  })
);
```
...
pipe(Maybe.make.some(42), Maybe.fold(() => 0, identity)); // 42
```
...
For complex object keys, you can supply a custom key-generator function to evaluate equivalence structurally, or specify a `maxSize` parameter to constrain memory usage via least-recently-used (LRU) eviction:

```ts
const loadProfile = memoize(
  (opts: { userId: string }) => fetchProfile(opts.userId),
  { key: (opts) => opts.userId, maxSize: 500 },
);
```
