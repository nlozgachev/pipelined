...
`Validation<E, A>` is either `Passed<A>` or `Failed<E>`. When you combine two
...
If both fields are failed, both errors come back in one result. Full guide:
[Validation — collecting errors](/guides/validation).

## Async operations with typed errors — `TaskResult<E, A>`

`try/catch` around `async/await` keeps errors untyped — callers can't tell from the return type
...
them work — start with [Thinking in pipelines](/basics/pipelines).
