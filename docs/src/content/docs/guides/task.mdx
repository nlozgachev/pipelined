...
const checkJob = (id: string): TaskResult<string, { status: "pending" | "done" }> =>
  TaskResult.tryCatch((signal) => fetch(`/jobs/${id}`, { signal }).then(r => r.json()), String);
...
Web applications cancel async work constantly: a component unmounts while a fetch is still in
flight, a user navigates away mid-poll, or a timeout fires on a slow endpoint. Without explicit
cancellation, the underlying request keeps running even though no one will consume the result —
wasting bandwidth, holding server connections open, and in the case of `retry` and `pollUntil`,
continuing to fire new requests that have already been abandoned.

`TaskResult.abortable` returns a plain `TaskResult` paired with an `abort` handle. When called,
`abort` fires an `AbortSignal` into the factory function. Pass it to `fetch` and the browser
cancels the in-flight request immediately:

```ts
const { task: fetchUser, abort } = TaskResult.abortable(
  (signal) => fetch(`/users/${id}`, { signal }).then((r) => r.json()),
  (e) => `Fetch failed: ${e}`,
);

// React component: cancel the request when the component unmounts
useEffect(() => {
...
The returned `task` is a plain `TaskResult`. It composes in a `pipe` chain just like any other,
and the signal threads through every combinator automatically — no extra wiring needed at the call
site:

```ts
const resilientFetch = pipe(
  fetchUser,
  TaskResult.retry({ attempts: 3, backoff: 1000 }),
  TaskResult.pollUntil({ when: (user) => user.status === "active", delay: 2000 }),
);

onCancel(abort);        // wire abort to whatever external event applies
await resilientFetch(); // abort reaches every retry attempt and every inter-poll delay
```

If cancellation fires during a backoff delay, `retry` stops immediately rather than starting
another request. `pollUntil` does the same — the inter-poll wait is cut short the moment the
signal fires.

#### Timeout that actually cancels

`TaskResult.timeout` doesn't just give up when the deadline fires — it passes an `AbortSignal` to
the inner operation so that the underlying request is actually cancelled rather than left running
silently in the background:

```ts
pipe(
  TaskResult.tryCatch(
    (signal) => fetch(`/reports/${id}`, { signal }).then((r) => r.json()),
    String,
  ),
  TaskResult.timeout(5000, () => new TimeoutError("report request timed out")),
);
// after 5 seconds: the fetch is aborted and Err(TimeoutError) is returned
// without the signal, the browser would keep the connection open and discard the result silently
```

#### Using your own controller

If you already hold an `AbortController` — from a framework hook, a test helper, or a parent
orchestrator — pass the signal at the call site instead of using `abortable`:
...
controller.abort();              // can be called before or after running
await result(controller.signal); // signal threads through every retry automatically
```

#### One `abortable` call = one cancellable unit

`abortable` bakes the `AbortController` into the task at creation time, not at invocation time.
Calling the same task twice in parallel shares one controller — `abort()` reaches both invocations:
...
task();
task();
// both tasks above use the same AbortController — abort() cancels them together
```

For requests that can be cancelled independently, create a separate `abortable` for each:

```ts
const { task: fetchUser,  abort: cancelUser  } = TaskResult.abortable(
  (signal) => fetch(`/users/${id}`, { signal }),
  String,
);
const { task: fetchPosts, abort: cancelPosts } = TaskResult.abortable(
  (signal) => fetch(`/posts?user=${id}`, { signal }),
  String,
);
```

#### Running requests in parallel with independent cancellation

Create one `abortable` per request, run them with `Task.all`, and cancel whichever one you
...
const [userResult, postsResult] = await Task.all([fetchUser, fetchPosts])();

cancelUser();  // only the user request is cancelled
cancelPosts(); // only the posts request is cancelled
```

The same principle applies to sequential requests. A common case is a search input that fires on
each keystroke: cancel the in-flight request before starting the next one, so only the latest
result is used. `TaskResult.restartable` handles this directly:

```ts
const { fn: search, abort } = TaskResult.restartable((query: string) =>
  TaskResult.tryCatch(
    (signal) => fetch(`/search?q=${query}`, { signal }),
    String,
  )
);

search("fo")().then(setResults);
search("foo")().then(setResults);   // cancels "fo", starts fresh
search("foob")().then(setResults);  // cancels "foo", starts fresh

abort(); // cancel without starting a new request — e.g. on component unmount
```

`restartable` holds one `AbortController` internally, replacing it on each call. The factory
always receives a fresh signal, and the previous one is aborted immediately.

#### Cancelling a multi-step chain

Some operations require several dependent requests in sequence: the first returns a URL, the next
follows it, and so on until the final response contains the data. `TaskResult.chain` handles this
naturally — and because `chain` forwards the call-site signal to each inner step, a single abort
reaches whichever request is currently in flight:
