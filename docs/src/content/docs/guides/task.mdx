...
## Cancelling in-flight work with `abortable`

`Task.abortable` wraps a factory function that receives an `AbortSignal` and returns a `Task` plus
a shared `abort` handle. Calling `abort()` cancels whichever call is currently in flight, and the
next call starts fresh:

```ts
const { task: searchUsers, abort } = Task.abortable(
  (signal) => fetch(`/users?q=${query}`, { signal }).then((r) => r.json()),
);

// User types quickly — each keystroke cancels the previous request
input.addEventListener("input", async (e) => {
  abort(); // cancel whatever is in flight
  const results = await searchUsers();
});
```

If you call `task()` while a previous call is still running, the previous call is aborted
automatically — you don't need to call `abort()` manually before each invocation. Both the explicit
`abort()` and an outer `AbortSignal` passed to `task(signal)` are respected.

## Running a task in a pipe chain

`Task.run` executes a task at the end of a `pipe` chain. Without it, a composed task needs
to be called with `task(signal)` outside the pipe — which splits the composition in two. `Task.run`
closes the pipe naturally:

```ts
const result = await pipe(
  loadConfig,
  Task.map(config => config.apiUrl),
  Task.run(),
);
```

Pass a signal to cancel the task from outside:
...
`repeat` and `repeatUntil` run a Task multiple times unconditionally — useful for scheduled polling
...
```ts
pipe(pollSensor, Task.repeat({ times: 5, delay: 1000 }))(); // Task<readonly Reading[]> — 5 readings, one per second
```
...
To avoid polling indefinitely, pass `maxAttempts`. When the limit is reached the last result is
returned regardless of whether the predicate passed:

```ts
pipe(
  checkDeploymentStatus,
  Task.repeatUntil({ when: (s) => s === "ready", delay: 2000, maxAttempts: 10 }),
)(); // gives up after 10 checks
```
...
`TaskResult.chain` forwards the call-site signal to each inner step automatically. A single
...
### Running a TaskResult in a pipe chain

`TaskResult.run` is the terminal step for a `TaskResult` pipe chain. It executes the task
and returns `Promise<Result<E, A>>`:
...
The signal threads through all tasks in the chain — abort at any point and whichever request
is in flight is cancelled immediately.
