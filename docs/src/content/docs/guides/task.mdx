...
const fetchReport = (reportId: string): Task.Result<string, Report> =>
  pipe(
    Task.Result.tryCatch((sig) => initiateJob(reportId, sig), String),
    Task.Result.chain((job) => Task.Result.tryCatch((sig) => checkJobStatus(job.id, sig), String)),
    Task.Result.chain((status) => Task.Result.tryCatch((sig) => downloadData(status.url, sig), String)),
  );
