...
By wrapping our async work in a function, we make it **lazy**—nothing runs until you explicitly invoke the Task. And by treating the core `Task` as infallible, we force all possible failures into the data channel: `Task.Result<E, A>` is a `Task` yielding a `Result<E, A>`, making failures impossible to overlook.
...
const fetchTimestamp = Task.from.Promise(() => Promise.resolve(Date.now())); // Task<number>
```
...
const fetchUserPreferences = (userId: string): Task<Preferences> =>
  Task.from.Promise(() => preferencesService.get(userId));
...
Because `Task` and `Task.Result` are lazy thunks, calling a task multiple times will re-trigger the underlying async computation every time.

`Task.memoize` (and `Task.Result.memoize`) creates a memoized task that executes the operation at most once on the first call and caches the resolved result for all subsequent calls:

```ts
const fetchToken = Task.memoize(
  Task.from.Promise(() => fetch("/auth/token").then((r) => r.json()))
);
...
*   **You want typed errors**: You want compiler-enforced error handling via `Task.Result` rather than catching untyped exceptions in standard `async/await` blocks.
