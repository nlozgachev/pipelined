...
`toFilter` converts a refinement into an `(a: A) => Maybe<B>` function, ready to drop into a
...
const parseSlug = pipe(
  ...
  Maybe.map(slug => `/posts/${slug}`),
  Maybe.getOrElse(() => "/posts/untitled"),
);
```
...
- You want to connect a runtime check to `Maybe` or `Result` pipelines using `toFilter` or
  `toResult`.
