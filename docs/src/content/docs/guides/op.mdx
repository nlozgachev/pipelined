...
Constructors (`Op.ok`, `Op.error`, `Op.nil`) and guards (`Op.isOk`, `Op.isError`, `Op.isNil`) are in
the API reference.
...
fetchUser.subscribe((map) => {
  for (const [userId, state] of map) {
    ...
    if (Op.isError(state))     showError(userId, state.error);
  }
});
```
...
`Op.wireAll` wires multiple source-handler pairs and returns a single cleanup:

```ts
const stop = Op.wireAll(
  [searchManager, (results) => displayManager.run(results)],
  [displayManager, (rendered) => analyticsManager.run(rendered)],
);
stop(); // removes all subscriptions at once
```
