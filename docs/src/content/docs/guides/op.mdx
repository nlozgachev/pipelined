...
happens — the call was cancelled, dropped, or replaced. These map to `Ok<A>`, `Error<E>`, and
`Nil`.
...
manager.subscribe((state) => {
  // state: Idle | Pending | Ok<User[]> | Error<ApiError> | Nil
...
// m1.subscribe: (state: Idle | Pending | Ok<A> | Error<E> | Nil) => void

// Retry present — Retrying is added
const m2 = Op.interpret(op, { strategy: "restartable", retry: { attempts: 3 } });
// m2.subscribe: (state: Idle | Pending | Retrying<E> | Ok<A> | Error<E> | Nil) => void
```
...
// Use in a subscriber
manager.subscribe((state) => {
  if (!Op.isOk(state) && !Op.isError(state) && !Op.isNil(state)) return;
  const result = pipe(
    state,
